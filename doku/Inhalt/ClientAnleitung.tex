...
\vspace{-10pt}
\end{figure}
Startet ein Benutzer die App erstmalig auf seinem Smartphone, so wird er von einem Willkommen-Dialog begrüßt. In diesem Dialog kann er die Karte auswählen, welche er nutzen möchte. Diese Auswahl kann später unter Einstellungen (siehe \ref{Einstellungen}) geändert werden. Zur Zeit steht für StudMap lediglich die Karte für die Westfälische Hochschule am Standort Bocholt zur Verfügung.
...
\subsection{Karte}
Die Karte wird über verschiedene Touch-Funktionen bedient. So kann mit der bekannten "`Pinch-to-Zoom"'-Geste in die Karte hinein bzw. hinaus gezoomt werden.
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{./Bilder/ClientGuide/ZoomEbene1.png}
\caption{Vergrößerte Ansicht von Ebene 1}
\label{fig:ZoomEbene1}
\end{figure}

\subsubsection{Start-/Zielpunkt wählen}
\label{ssec:StartZielpunkt}

Um einen Punkt als Start- oder Zielpunkt zu markieren, öffnet sich nach der Anwahl eines Punktes ein Dialog mit entsprechenden Auswahlmöglichkeiten.
\begin{figure}[H]
\centering
...
Markierte Punkte werden zunächst vergrößert und rot dargestellt.
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{./Bilder/ClientGuide/EinblendungStartpunkt.png}
\caption{Vergrößerte Ansicht des Dialogs für einen Startpunkt}
\label{fig:Startpunkt}
\end{figure}

\subsubsection{Navigation}
Wurde ein Start- und ein Zielpunkt gewählt (siehe \nameref{ssec:StartZielpunkt}), wird eine Route automatisch berechnet und eingezeichnet. Start und Ziel bekommen jeweils ein ansprechendes Symbol und unten im Bild ist eine Einblendung über Start und Ziel.
\begin{figure}[H]
\centering
...
\subsubsection{Suche}
Bei der Suchfunktion kann über ein Suchfeld, welches sich selbst vervollständigt (\ref{fig:Suchen}, rechtes Bild), ein gesuchter Raum gefunden werden.
\begin{figure}[H]
\centering
...
Wird ein Punkt durch die Suche oder den PoI-Dialog (siehe \nameref{PoIDialog}) markiert, so wird dieser leicht vergrößert und rot dargestellt. Im unteren Bildschirm wird zusätzlich ein Dialog mit den Knoteninformationen eingeblendet.
\begin{figure}[H]
\centering
...
Die Positionierung kann auf zwei Arten erfolgen. Zum einen kann ein NFC-Tag eingescannt werden, dies erfolgt im Hintergrund und es ist keine Benutzeraktion notwendig. Zum anderen kann ein QR-Code eingescannt werden, wozu zunächst über das entsprechende Icon der Scanner gestartet werden muss.

\subsubsection{Neuladen}
Sollte die Karte einmal nicht erreichbar sein oder nur fehlerhaft geladen werden, so kann hier das Laden der Karte manuell angestoßen werden.
...
\caption{Ansicht des Menüs}
\label{fig:Menue}
\end{figure}

\subsubsection{Ebene auswählen}
...
\caption{Dialog für die Anmeldung}
