...
\caption{Datenbankmodell für die Kartenobjekte}
\label{fig:MapEntities}
\end{figure}

\subsection{Maps}
\label{db:maps:Maps}
Für jede Karte wird ein Eintrag in dieser Tabelle erzeugt.
...
\subsection{Floors}
Für jedes Stockwerk wird ein Eintrag in dieser Tabelle angelegt.
...
Dazu werden alle Kanten mit dem Graphen verknüpft. Über die Kanten
sind auch die Knoten mit dem Graphen indirekt verknüpft.
...
und seine X/Y-Koordinaten auf diesem Stockwerk bestimmt. Außerdem wird
...
Zu einem Knoten können noch weitere Informationen hinterlegt werden.
Diese sind optional und werden nur zu wichtigen Knoten wie
Seminarräumen, Büros und Toiletten hinzugefügt. Über die Knoteninformationen
kann auch ein PoI mit dem Knoten verknüpft werden.
...
\subsection{PoIs}
\label{db:maps:PoIs}
Ein PoI (Point of Interest) kategorisiert für den Benutzer relevante Knoten.
Hier kann z.B. nach Dozentenbüros, Mensa, Bibliothek und Toiletten gefiltert
werden.
...
\hline Description			& NVARCHAR(MAX)		 & Zusätzliche Beschreibung des PoIs \\  
\hline 
\end{tabularx} 
